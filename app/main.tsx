...
export default function Main() {
    ...
    async function handleConfirmarPresenca() {
        ...
        const { error } = await supabase.from(table).update({ status: 'confirmado' }).eq('id', next.id);
        setConfirming(false);
        if (error) { Alert.alert('Erro', 'Não foi possível confirmar presença.'); return; }
    ...
    return (
        <SafeAreaView style={style.safeArea}>
            ...
            <ScrollView style={style.scroll} contentContainerStyle={style.scrollContent} showsVerticalScrollIndicator={false}>
                ...
                <View style={style.modalOverlay}>
                    <View style={style.modalSheet}>
                        ...
                        <Text style={style.modalTitle}>
                            {isConfirmado ? '✅ Presença confirmada' : `${next?.kind === 'consulta' ? 'Consulta' : 'Exame'} agendado`}
                        </Text>
                        <View style={style.modalDivider} />
                        <View style={style.modalRow}>
                            <View style={style.modalRowIcon}>
                                <MaterialCommunityIcons name={next?.kind === 'consulta' ? 'stethoscope' : 'flask-outline'} size={18} color={themas.color.primary} />
                            </View>
                            <View>
                        ...
                        <View style={style.modalDivider} />
                        {!isConfirmado && (
                            <TouchableOpacity style={style.modalConfirmBtn} onPress={handleConfirmarPresenca} disabled={confirming}>
                                {confirming
                                    ? <ActivityIndicator color="#fff" />
                                    : <><Ionicons name="checkmark-circle-outline" size={20} color="#fff" /><Text style={style.modalConfirmText}>Confirmar presença</Text></>}
                            </TouchableOpacity>
                        )}
