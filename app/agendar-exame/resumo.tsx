...
import { style } from '../../src/pages/agendar-exame/resumo';
...
export default function ResumoExame() {
    const router = useRouter();
    const { nome, cpf, exame, data, horario } = useLocalSearchParams<{
        nome: string; cpf: string; exame: string; data: string; horario: string;
    }>();
    ...
    async function handleConfirmar() {
        ...
        const { error } = await supabase.from('exams').insert({
            ...
            patient_name: nome,
            cpf, type: exame,
            date: data, time: horario, price: 120,
        });
        ...
        <SafeAreaView style={style.safeArea}>
            ...
            <ScrollView contentContainerStyle={style.scrollContent} showsVerticalScrollIndicator={false}>
                ...
                <View style={style.card}>
                    <Text style={style.cardTitle}>Detalhes do agendamento</Text>
                    <DetailRow lib="ion" icon="person-outline" label="Paciente" value={nome} />
                    <DetailRow lib="mci" icon="flask-outline" label="Tipo de exame" value={exame} />
                    <DetailRow lib="ion" icon="calendar-outline" label="Data e Horário"
                        value={data} sub={`${horario} - Duração aprox. 30 min`} highlight />
                    <DetailRow lib="ion" icon="cash-outline" label="Valor do exame" value="R$ 120,00" />
                </View>
                ...
                <View style={style.recomCard}>
                    ...
                    {[
                        ...
                        'Evitar comer até 3 horas antes do exame',
                    ].map((r, i) => (
